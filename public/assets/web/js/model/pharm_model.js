const pharm_m = {
    ...
    /*
         Use > 매입처 정보 등록
         params > vename : 회사명,vudesc:상세정보,veemail : 회사이메일, vebusino : 사업자등록번호,vubusiemail : 세금계산서이메일,vubusizip : 사업자우편번호,vubusiaddr1 : 사업자주소1,vubusiaddr2 : 사업자주소2,vubusitel : 담당자전화번호
         return > effect : 처리 갯수
         */
    ...
    /*
         Use > 원자재 리스트
         params > pcnt : 페이지당갯수, page : 현재 페이지,skey:약재명검색
         return > list : 데이터 , total:갯수, totalRs :전체갯수,nPage:다음페이지
         */
    ...
    /*
         ...
         return > list : 데이터 , total:갯수, totalRs :전체갯수,nPage:다음페이지
         */
    async Load_Pharm_Material_TradeList(params) {
        const res = await commonRequest('/Load_Pharm_Material_TradeList', params);
        if (!res) return { list : [],total : 0,totalRs : 0,nPage :0};
        const [item = {}] = res.data || [];
        return {
            list: item.list || [],
    ...
    return > list : 데이터, tcnt : 갯수
     */
    async Load_Pharm_Medicine_Product(params){
        const res = await commonRequest('/Load_Pharm_Medicine_Product',params);
        if (!res) return { list: [], total: 0 };
        const [item = {}] = res.data || [];
        return  {
            list : item.list,
    ...
    /*
     Use > 약재 입,출고
     params > hncode : 약재코드,typ:(1:입고,2출고), stock : 입출고량, reason : 사유,vcode : 매입처코드or구입처코드, v_price : 매입가격or구입가격,v_unitprice : 매입단가or구입가격,in_memo : 메모,indate : 매입일자
     return > list : 데이터 , total:로드된 데이터 수
     */
    async Input_Pharm_Medicine_InOut(params) {
        const res = await commonRequest('/Input_Pharm_Medicine_InOut', params);
        if (!res) return { ecnt: 0 };
        const [item = {}] = res.data || [];
        return  {
            effect : item.ecnt || 0
    ...
    /*
         ...
         return > list : 데이터 , total:갯수, totalRs :전체갯수,nPage:다음페이지
         */
    async Load_Pharm_Medicine_TradeList(params) {
        const res = await commonRequest('/Load_Pharm_Medicine_TradeList', params);
        if (!res) return { list : [],total : 0,totalRs : 0,nPage :0};
        const [item = {}] = res.data || [];
        return {
            list: item.list || [],
