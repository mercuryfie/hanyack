...
class Board_m extends Model
{
    ...
    public function Load_Board_Env ($bid,$fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
        $sql = "SELECT {$separated_val} FROM han_board_env WHERE bid=:BID: AND isOpen=1";
        $bindparam = [
            'BID' => $bid
        ];
    ...
    public function Load_Board ($bid,$fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
        $sql = "SELECT {$separated_val},(select userid from herb_member where uid=a.uid) as userid FROM han_board a WHERE bid=:BID: and isDel=0 order by bcode DESC";
        $bindparam = [
            'BID' => $bid
        ];
        $Query = $this->db->query($sql, $bindparam);
    ...
    public function Load_Board_Page ($param,$fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
        $sql = "SELECT {$separated_val},(select userid from herb_member where uid=a.uid) as userid FROM han_board a WHERE bid=:BID: and isDel=0 order by bcode DESC LIMIT :LIMIT: OFFSET :OFFSET:";
        $bindparam = [
            'BID' => $param['bid'],
            'LIMIT' => $param['limit'],
            'OFFSET' => $param['offset']
    ...
    public function Load_Board_Contents ($bcode, $fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
        $sql = "SELECT {$separated_val} FROM han_board WHERE bcode=:BCODE: and isDel=0";
        $bindparam = [
            'BCODE' => $bcode
        ];
        $Query = $this->db->query($sql, $bindparam);
    ...
    public function Load_Board_RContent ($bcode,$fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
    ...
    public function Load_Board_Reply ($bcode,$fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
        $sql = "SELECT {$separated_val} FROM han_board_reply WHERE isDel=0";
        $bindparam = [
            'FK_BCODE' => $bcode
        ];
        $Query = $this->db->query($sql, $bindparam);
    ...
    public function Load_Board_Attachment ($fk_bcode,$fields=['ALL']){
        $separated_val = fn_Make_Fields($fields);
