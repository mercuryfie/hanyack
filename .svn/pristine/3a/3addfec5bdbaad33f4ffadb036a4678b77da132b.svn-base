            ...
            if (Cnt > 0) {
                $.each(arr, function (index, el) {
                    ...
                    if (el.p_type == 0) {
                        ...
                        p_type_str = '출고시작';
                        delcode_html = Make_delcode(el.sn);
                        subHhtml = `
                            ${delcode_html}
...
async function Load_Package(pcode){
    try {
        ...
        if (result.get('status') == 'NoLogin') {
            ...
            let Cnt = (arr !== undefined && arr !== null) ? arr.length : 0;
            if (Cnt > 0) {
                $.each(arr, function (index, el) {
                    html += `
                        <tr>
                            <td><input type="checkbox" name="chkorder" value="${el.sn}"/></td>
                            <td>${el.pa_code}</td>
                            <td>${el.fk_pcode}</td>
                            <td>${el.fk_hncode}</td>
                            <td>${el.hn_name}</td>
