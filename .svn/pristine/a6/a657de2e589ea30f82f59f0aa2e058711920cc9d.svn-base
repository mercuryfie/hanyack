$(document).ready(function() {
    ...
    $('#submitBtn').on('click',function () {
        ...
        let $row = $('tr[name="addEvent"]');
        if ($row.is(':empty')) {
            Make_Toast('타입을 선택하십시오.');
        } else {
            add_NewEvent();
            // submit_EventType();
...
function submit_EventType2(dataarr) {
    console.log('this',dataarr);
}

// async function submit_EventType2(hncode) {
//     console.log('this111',hncode);
//     try {
//         start_spinner();
//         let url = APIURL + '/Insert_ProductEvent';
//         let result = await Load_API2(url,dataarr);
//         if(result.get('status')=='NoLogin'){
//             go_login();
//         }else if(result.get('status')=='ok'){
//             location.reload();
//             Make_Toast('이벤트 타입을 등록했습니다.');
//         }else{
//             Make_Toast('2이벤트 타입을 등록했습니다.');
//             alert(result.get('message'));
//         }
//         stop_spinner();
//     } catch (error) {
//         Make_Toast('3이벤트 타입을 등록했습니다.');
...
async function submit_EventType() {
    try {
        console.log('this111',);
        let result1 = await submit_EventType_data();
...
function submit_EventType_data(){
    return new Promise(function(resolve, reject) {

        let formdata = new FormData($("#boardForm")[0]);

        formdata.append("ftype", $('[name="ftype"]').val());
        formdata.append("sn", $('[name="sn"]').val());
        formdata.append("hncode", $('[name="hncode"]').text());
        formdata.append("hnname", $('[name="hnname"]').text());
...
async function setEvent3(dataarr) {
    try {
        start_spinner();
...
// async function add_NewEvent() {
//
//     try {
//         let result = await add_NewEvent_Data();
//         if(result.status == 'ok') {
//             Make_Toast('저장하였습니다.');
//             do_refresh();
//             console.log('bello1');
...
// async function Load_ProductEvent(hncode) {
//     console.log('this111',dataarr);
//     try {
//         start_spinner();
...
function setEvent2(miname,hncode, hnname){
    Ini_popEvent();
    $('#miname').text(miname);
    $('#hncode').text(hncode);
    $('#hnname').text(hnname);
...
async function setEvent(miname,hncode, hnname) {
    ...
    try {
        start_spinner();
        let result = await setEvent_Data(miname,hncode, hnname);
        if(result.status == 'ok') {
            $('#eventPop').show();
            $('#miname').text(miname);
            $('#hncode').text(hncode);
            $('#hnname').text(hnname);
            $('#eventTable').append(result.data);
            console.log('bello1');
        }
        stop_spinner();

    } catch (error) {
        Make_Toast('Oops! somehing went wrong.');
        console.log('bello333333');
        console.log( error );
        stop_spinner();
    }
...
function setEvent_Data(miname,hncode, hnname){
    return new Promise(function(resolve, reject) {

        console.log('$root:', miname,hncode, hnname);
        let formdata = new FormData($("#thisForm")[0]);

        formdata.append("hncode", hncode);
        ...
        $.ajax({
            ...
            success : function(response) {
                ...
                if(response.result=='ok' && response.info.length>0){
                    ...
                    resolve({ status: 'ok',message:'success', data: html });
                }else{
                    retarr['status'] = 'error';
                    retarr['message'] = response.message;
                    reject(retarr);
...
function addEvent() {
    let value = $('#eventType').val();
    let value2 = $('[name="ftype"]').val();
    let hncode = $('#hncode').text();
    let hnname = $('#hnname').text();
    console.log('titi',value,hncode,hnname,value2);

    if ($('[data-type="' + value + '"]').length > 0) {
        Make_Toast('이미 등록된 타입입니다.');
        ...
        if (value == 1) {
            let html = '';
            html += ` 
            <tr data-add="addEvent" class="addEvent">
                <input type="hidden" name="ftype[]" value="1">
                <td class="hncode" name="hncode" data-hncode="`+hncode+`">`+hncode+`</td>
                <td class="hnname"  >`+hnname+`</td>
                <td class="" data-type="1">인기 약재</td>
                <td class="delete">
                    <i class="fa-solid fa-xmark " id="" data-remove="removeBtn"></i>
                </td> 
            </tr>
        `;
...
async function add_NewEvent() {
    try {
        ...
        Make_Toast('2Oops! somehing went wrong.');
        console.log('bello333333');
        console.log( error );
    }
}
...
function add_NewEvent_Data(){
    return new Promise(function(resolve, reject) {
        ...
        let $root = $('#eventTable');
        let f_arr = $root.find('[name="ftype"]').map(function(){
            return $(this).val();
            }).get();
        console.log('titi2',hncode,f_arr);

        formdata.append("hncode", hncode);
        formdata.append("ftype", f_arr);

        let retarr = new Array();
        for (let pair of formdata.entries()) {
            console.log('bello 12',pair[0]+ ': ' + pair[1]);
        }

        $.ajax({
            url: APIURL + '/Insert_ProductEvent',
            type : 'POST',
            data: formdata,
            dataType: "JSON",
            cache : false,
...
async function Load_Herb(page, skey) {
    try {
        start_spinner();
        let dataarr = {"page": page, "skey": skey,"typ" : 100};
        let url = APIURL + '/Load_herbList';
        let result = await Load_API(url,dataarr);
        console.log(dataarr);
        if (result.get('status') == 'NoLogin') {
            ...
            if (Cnt > 0) {
                $.each(arr, function (index, el) {
                    ...
                    let ctrBtn =`<button type="button" 
                        onclick="setEvent('${el.mi_name}','${el.hn_code}','${el.hn_name}');" 
                        class="btnType1" name="ctrBTn">설정</button>`;
                    ...
                    html += `
                        <tr id="line_${el.sn}">
                            ...
                            <td class="authStatus" ${authStatus_str}>${el.hn_isok_str}</td>
                            <td>${el.hn_code}</td>
                            <td>${el.mi_name}</td>
                            <td>${el.hn_name}</td>
                            <td>${el.fk_mdname}</td>
                            ...
                            <td>${number_format(el.hn_stock)}개</td>
                            <td>${ctrBtn}</td>
                            <td class="preview">
                                ${subimg}
                                ${subfile}
