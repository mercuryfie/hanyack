<?php

namespace App\Controllers;

use App\Libraries\Form;
use App\Libraries\Utils;

class OrderController extends BaseController
{
    public function __construct()
    {
        $Auth = [AUTH_MASTER,AUTH_DECOC];
        $this->Check_Auth($Auth);
    }

    public function NewSmartOrder(){
        $sessinarr = $this->GetSessionData();
        if(!$sessinarr['islogin']){
            return redirect()->to('/Member/Login')->with('msg','로그인이 필요합니다.');
        }else {
            $mi_code = $sessinarr['user']['mi_code'];

            $order_m=model('Order_m');
            $Rs =$order_m->Load_Company($mi_code);
            $cfcode = $Rs[0]['mi_cfcode'];
            $s_data = [];

            $metaarr = [
                'h_title' => '스마트 오더',
                'h_type' => 1
            ];

            $body = [
                'cfcode' => $cfcode
            ];


            $form = new Form;
            $main_data = [
                'meta' => $form->fnMake_Meta($metaarr),
                'header' => $form->fnMake_Header($sessinarr),
                'search' => $form->fnMake_Search($s_data),
                'body' => $body
            ];

            return view('web/decoc/newsmartOrder_View', $main_data);
        }
    }


    public function SmartOrder(){
        $sessinarr = $this->GetSessionData();
        if(!$sessinarr['islogin']){
            return redirect()->to('/Member/Login')->with('msg','로그인이 필요합니다.');
        }else {
            $mi_code = $sessinarr['user']['mi_code'];

            $order_m=model('Order_m');
            $Rs =$order_m->Load_Company($mi_code);
            $cfcode = $Rs[0]['mi_cfcode'];
            $s_data = [];

            $metaarr = [
                'h_title' => '스마트 오더',
                'h_type' => 1
            ];

            $page =

            $body = [
                'cfcode' => $cfcode
            ];


            $form = new Form;
            $main_data = [
                'meta' => $form->fnMake_Meta($metaarr),
                'header' => $form->fnMake_Header($sessinarr),
                'search' => $form->fnMake_Search($s_data),
                'body' => $body
            ];

            return view('web/decoc/smartOrder_View', $main_data);
        }
    }

    public function OList(){
        $sessinarr = $this->GetSessionData();
        $s_data = [];

        $metaarr = [
            'h_title' => '상품리스트',
            'h_type' => 1
        ];

        $form = New Form;
        $main_data = [
            'meta' => $form->fnMake_Meta($metaarr),
            'header' => $form->fnMake_Header($sessinarr),
            'search' => $form->fnMake_Search($s_data)
        ];
        
        return view('web/myOrderListDec_View',$main_data);
    }

    public function Cart(){
        $sessinarr = $this->GetSessionData();
        if(!$sessinarr['islogin']){
            return redirect()->to('/Member/Login')->with('msg','로그인이 필요합니다.');
        }else {
            $mi_code = $sessinarr['user']['mi_code'];
            $address_arr = $this->LoadAddress($mi_code);
            if(fn_ArrayCnt($address_arr)>0){
                $address = $address_arr['address1'] . ' ' . $address_arr['address2'];
            }else{
                $address = '';
            }
            $s_data = [];
            $metaarr = [
                'h_title' => '장바구니',
                'h_type' => 1
            ];
            $left_menu = [];
            $body_arr = [
                'address' => $address
            ];

            $form = new Form;
            $main_data = [
                'meta' => $form->fnMake_Meta($metaarr),
                'header' => $form->fnMake_Header($sessinarr),
                'search' => $form->fnMake_Search($s_data),
                'left_menu' => $left_menu,
                'body' => $body_arr

            ];

            return view('web/decoc/cart_View', $main_data);
        }
    }
}
